...
# world <- sf::st_read('TM_WORLD_BORDERS-0.3/TM_WORLD_BORDERS-0.3.shp')

data(wrld_simpl)
world_simple <- wrld_simpl
# plot(world_simple)
# Using hate incident stats by OSCE's ODIHR
osce_data <- read_csv(file = 'data/2018/osce_hate_incidents_2018_wide.csv')
osce_data

# plot1=======
smaller_data <- osce_data %>% 
    dplyr::select(iso3c, other_sources) %>% 
    na.omit() %>% 
    data.frame()

## Join the two datasets using their common field
# 
world_simple@data <- world_simple@data %>% 
   left_join(smaller_data, by=c("ISO3"="iso3c"))

matched_indices <- match(wrld_simpl@data[, "ISO3"], smaller_data[, "iso3c"])
wrld_simpl@data <- data.frame(wrld_simpl@data, smaller_data[matched_indices, ])
...
# world.carto_civil <- cartogram_cont(wrld_simpl, "other_sources", itermax=5)

# plot(world.carto_civil)
world.carto_civil <- quick.carto(world_simple, world_simple@data$other_sources, blur = 0.5)
plot(world.carto_civil)

## Convert the object into data frame
world.f_civil <- fortify(world.carto_civil, 
                         # region = "Country.Code"
                         )
...
ggsave(plot = carto_civil,filename = "viz/2018/Cartogram_002_civilorg_2018.png", width = 16*0.8, height = 9*0.8, units = "in" , dpi = 300, scale = 1.5)
...
ggsave(filename = "viz/2018/Cartogram_002_official_2018.png",plot = carto_official,
...
carto_civil_wo_labels <- ggplot(world.f2, aes(long, lat, group = group, fill = world.f2$hate_fig_civil %>% log2))+
    ...
    labs(x="Distorted Longitude", 
         y='Distorted Latitude', 
         title="Cartogram of Racist and Xenophobic Incidents Reported by Other Sources in 2018",
         subtitle="Source: Office for Democratic Institutions and Human Rights (ODIHR) of the\nOrganization for Security and Co-operation in Europe (OSCE)  ",
         caption="Social Data Science Lab, Cardiff University")+
    hrbrthemes::theme_ipsum_rc()+
    theme(plot.caption = element_text(size = 12))+
...
ggsave("viz/2018/Cartogram_002_civil_wo_labels_2018.png", width = 16*0.8, height = 9*0.8, units = "in" , dpi = 300, scale = 1.5)
...
ggsave(filename = "viz/2018/Cartogram_002_official_wo_labels.png",plot = carto_official_wo_labels,
       width = 16*0.8, height = 9*0.8, units = "in" , dpi = 300, scale = 1.5)
...
ggsave("viz/2018/Cartogram_002_civil_countrycodes_2018.png", width = 16*0.8, height = 9*0.8, units = "in" , dpi = 300, scale = 1.5)
...
ggsave("viz/2018/Cartogram_002_official_countrycodes_2018.png", width = 16*0.8, height = 9*0.8, units = "in" , dpi = 300, scale = 1.5)
