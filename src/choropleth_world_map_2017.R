...
osce_data <- read_csv("data/2017/osce_hate_incidents_2017_wide.csv") #reading in OSCE hate incidents
...
plot3 <- ggplot()+ 
    ...
    labs(x="Longitude", 
         y='Latitude', 
         title="Racist and Xenophobic Incidents Reported by Other Source in 2017",
         subtitle="Source: Office for Democratic Institutions and Human Rights (ODIHR) of the\nOrganization for Security and Co-operation in Europe (OSCE)  ",
         caption="Social Data Science Lab, Cardiff University")+
    hrbrthemes::theme_ipsum_rc()+
    theme(plot.caption = element_text(size = 10))+
...
plot6 <- ggplot()+ 
    # theme(legend.position="bottom")+ 
    geom_map(data=osce_map_joined, map=map.world, 
             aes(map_id=region, x=long, y=lat, fill=osce_map_joined$other_sources %>% log2))+
    ...
    scale_fill_gradientn (name="Incident Counts",
                          ...
                          guide = "colourbar",
                          breaks= rep(1:10),
                          labels=c(1,5, 10, 25,50, 100, 250, 500, 1000,1500)
                          )+
    coord_map(xlim=c(-180,180), ylim = c(-60, 150))+
    labs(x="Longitude", 
         y='Latitude', 
...
ggsave(filename = "viz/2017/Choropleth_002_civilorg_2017.png",
       plot = plot6,
